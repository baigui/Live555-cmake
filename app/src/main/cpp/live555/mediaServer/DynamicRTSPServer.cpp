...
::lookupServerMediaSession(char const* streamName, Boolean isFirstLookupInSession) {
  ...
  char fullpath[PATH_MAX] = "/sdcard/";
  strcat(fullpath,streamName);
    __android_log_print(ANDROID_LOG_ERROR, "VFConfig", "'%s'\n", fullpath);
  FILE* fid = fopen(fullpath, "rb");

  Boolean fileExists = fid != NULL;

  // Next, check whether we already have a "ServerMediaSession" for this file:
  ServerMediaSession* sms = RTSPServer::lookupServerMediaSession(fullpath);
  ...
  if (!fileExists) {
    ...
    __android_log_print(ANDROID_LOG_ERROR, "VFConfig", "file exit,yes\n");
